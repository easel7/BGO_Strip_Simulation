...
void Percentile2DepthEst_Mono()
{
    ...
    auto Acc_p = new TGraph();  
    auto Acc_d = new TGraph();
    auto D68_p = new TGraph();
    auto D68_d = new TGraph();
    ...
    // for (int k =23; k < 24; k++)
    {
        ...
        auto h1_p_Dep = new TH1D("h1_p_Dep",";Depth (mm);Normalized Count",200,0,100);; // h1_p_Dep 装 Est - True
        auto h1_d_Dep = new TH1D("h1_d_Dep",";Depth (mm);Normalized Count",200,0,100);; // h1_d_Dep 装 Est - True
        ...
        {        
            ...
            h1_p_Lay[p_FI_Lay]->Fill( (p_FI_Dep -  Est_Depth) / p_FI_Dep );
            h1_p_Dep->Fill( abs(p_FI_Dep -  Est_Depth));
        }
        ...
        for (Long64_t entry = 0; entry < deuteron_tree->GetEntries() ; ++entry)
        {
            ...
            h1_d_Dep->Fill( abs(d_FI_Dep -  Est_Depth));
        }
        ...
        auto c2 = new TCanvas("c2","c2",1600,1600);
        c2->cd();
        gStyle->SetOptStat(0);
        h1_p_Dep->SetLineColor(kRed);
        h1_d_Dep->SetLineColor(kBlue);
        h1_p_Dep->Scale(1/h2_p_tot->Integral());
        h1_d_Dep->Scale(1/h2_d_tot->Integral());
        h1_p_Dep->Draw("hist");
        h1_d_Dep->Draw("histsame");

        double quantiles[3] = {0.68, 0.90, 0.95};  // Percentiles
        double value_p[3] = {0};
        double value_d[3] = {0};
        h1_p_Dep->GetQuantiles(3,value_p,quantiles);
        h1_d_Dep->GetQuantiles(3,value_d,quantiles);
        Acc_p->AddPoint(Energy[k],h1_p_Dep->Integral(1,30)*100);
        Acc_d->AddPoint(Energy[k],h1_d_Dep->Integral(1,30)*100);
        D68_p->AddPoint(Energy[k],value_p[0]);
        D68_d->AddPoint(Energy[k],value_d[0]);
        cout << h1_p_Dep->Integral(1,30) << " , " << h1_d_Dep->Integral(1,30) << endl;

        auto legend2 = new TLegend(0.7,0.7,0.88,0.88);
        legend2->AddEntry(h1_p_Dep,"Proton","l");
        legend2->AddEntry(h1_d_Dep,"Deuteron","l");
        legend2->Draw();
        ...
        auto c2_0 = new TCanvas("c2_0","c2_0",1600,800);
        c2_0->Divide(2,1);
        c2_0->cd(1);
        gPad->SetLogx();
        gPad->SetGrid(1,1);
        Acc_p->SetTitle(";Incident Energy (GeV);Accuracy (%)");
        Acc_p->GetYaxis()->SetRangeUser(0,100);
        Acc_p->GetXaxis()->SetLimits(9e0,2e4);
        Acc_p->SetLineColor(kRed);
        Acc_d->SetLineColor(kBlue);
        Acc_p->SetMarkerColor(kRed);
        Acc_d->SetMarkerColor(kBlue);
        Acc_p->SetMarkerStyle(20);
        Acc_d->SetMarkerStyle(21);
        Acc_p->Draw("ALP");
        Acc_d->Draw("PLSAME");
        legend2->Draw();
        c2_0->cd(2);
        gPad->SetLogx();
        gPad->SetGrid(1,1);
        D68_p->SetTitle(";Incident Energy (GeV);D_{68} (mm)");
        D68_p->GetYaxis()->SetRangeUser(0,50);
        D68_p->GetXaxis()->SetLimits(9e0,2e4);
        D68_p->SetLineColor(kRed);
        D68_d->SetLineColor(kBlue);
        D68_p->SetMarkerColor(kRed);
        D68_d->SetMarkerColor(kBlue);
        D68_p->SetMarkerStyle(20);
        D68_d->SetMarkerStyle(21);
        D68_p->Draw("ALP");
        D68_d->Draw("PLSAME");
        legend2->Draw();
