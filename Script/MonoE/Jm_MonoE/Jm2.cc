void Jm2()
{
    ...
    for (int i = 18; i < 19; i++) // Energy
    {
        ...
        for (int j = 0; j < 14; j++)
        {
            h1_p[j] = new TH1D(Form("h1_p[%d]",j), Form("h1_p[%d]",j),100,0,1);  
            h1_d[j] = new TH1D(Form("h1_d[%d]",j), Form("h1_d[%d]",j),100,0,1);  
        ...
        // for (Long64_t entry = 0; entry < 1; ++entry)
        {
            ...
            for (size_t k = 0; k < p_EnergyVec->size(); k += 22)
            {
                ...
                // if((*p_RMSVec)[0]>15 && (*p_RMSVec)[1]>15) 
                {
                    ...
                    if ((*p_L_EnergyVec)[index] > 0 && sum_p > 0)
                    {
                        hC_p[index]->Fill(sum_p / sum_p_total);
        ...
        for (int j = 0; j < 14; j++)
        {
            ...
            hC_p[j]->SetTitle(Form("%.f GeV incident;#sum_{0}^{%d} Max Energy Deposit bar/#sum_{0}^{%d} Total Deposit",Energy[i],j,j));
            hC_p[j]->GetYaxis()->SetRangeUser(0,0.1);
            hC_p[j]->Draw("hist");
            ...
            h1_p[j]->GetYaxis()->SetRangeUser(0,0.1);
            h1_p[j]->SetTitle(Form("%.f GeV incident in L%d;Jm = Max Energy Deposit bar/ Total Deposit;Normalized Count",Energy[i],j));
            h1_p[j]->Draw();
            h1_p[j]->Draw("hist");
        ...
        c2->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Jm_MonoE/CDF/Jm_DP_%dGeV.pdf",int(Energy[i])));
        ...
        gre_d->SetTitle(Form("Incident Energy %d GeV ; BGO Layer; Jm = (dE/dx) / Edep",int(Energy[i])));
        ...
        auto axis_top = new TGaxis(0, 1, 14, 1, 0, 32, 510, "-L"); 
        ...
        legend3->Draw();
        c3->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Jm_MonoE/CDF/Jm_BGOLayer_DP_%dGeV.pdf",int(Energy[i])));
        ...
        SUM_gre_d->SetTitle(Form("Incident Energy %d GeV ; BGO Layer; #sum Max Energy Deposit bar/#sum Total Deposit",int(Energy[i])));
        ...
        legend4->Draw();
        c4->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Jm_MonoE/CDF/Jm_BGOLayer_SUM_DP_%dGeV.pdf",int(Energy[i])));


        auto c5 = new TCanvas("c5", "c5", 1500, 1000);
        gPad->SetLogy(1);
        gr_proton->SetTitle(Form("Incident Energy %d GeV ; BGO Layer; #sum Max Energy Deposit bar/#sum Total Deposit",int(Energy[i])));
        gr_proton->GetYaxis()->SetRangeUser(1e-1,1);
        ...
        legend5->Draw();
        c5->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Jm_MonoE/CDF/Jm_BGOLayer_SCAT_DP_%dGeV.pdf",int(Energy[i])));

        auto c6 = new TCanvas("c6", "c6", 1500, 1000);
        h2_p->SetTitle(Form("Incident Energy %d GeV ; BGO Layer; #sum Max Energy Deposit bar/#sum Total Deposit",int(Energy[i])));
        h2_p->GetYaxis()->SetRangeUser(-1.2,0.2);
        ...
        auto SUM_axis_top = new TGaxis(0, 0, 14, 0, 0, 32, 510, "-L"); 
