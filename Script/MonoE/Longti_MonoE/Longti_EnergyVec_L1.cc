void Longti_EnergyVec_L1()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        for (size_t i = 22*k; i < 22*(k+1); i += 22)
        {
            ...
            if(k>0 && k<13)
            {
                h1_p->Fill(log10((*p_L_EnergyVec)[k]));
                if(p_First_Had_Layer==k) { h1_d->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer==-1) { h1_h->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=floor((k-1)/2) && p_First_Had_Layer<=k-1             && p_First_Had_Type == 1)  { h1_e->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=0              && p_First_Had_Layer<floor((k-1)/2)   && p_First_Had_Type == 1)  { h1_H->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer<=k-1 && p_First_Had_Type == 2)                                                   { h1_c->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=k+1 && p_First_Had_Type == 1 )                                                  { h1_E->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=k+1 && p_First_Had_Type == 2 )                                                  { h1_C->Fill(log10((*p_L_EnergyVec)[k])); }
            }
            else if (k==0)
            {
                h1_p->Fill(log10((*p_L_EnergyVec)[k]));
                if(p_First_Had_Layer==0)                                                                             { h1_d->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer==-1)                                                                            { h1_h->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=1 && p_First_Had_Type == 1)                                                    { h1_e->Fill(log10((*p_L_EnergyVec)[k])); }
                if(p_First_Had_Layer>=1 && p_First_Had_Type == 2)                                                    { h1_c->Fill(log10((*p_L_EnergyVec)[k])); }
            }
            else if (k==13)
            {
                h1_p->Fill(log10((*p_L_EnergyVec)[k]));
    ...
    h1_p->GetYaxis()->SetRangeUser(0,2000);h1_p->SetTitle(Form("1000GeV incident EdepRatio Distrubution in L%d;log_{10}(Energy Deposit in L%d/GeV);Normalized Count",k,k));
