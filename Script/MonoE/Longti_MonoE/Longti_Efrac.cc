void Longti_Efrac()
{
    ...
    for (int i = 0; i < 19; i++) // Energy
    {
        ...
        for (int j = 0; j < 14; j++)
        {
            ...
            {
                proton_tree->GetEntry(entry);   if ((*p_RMSVec)[0] > 15 && (*p_RMSVec)[1] > 15) h1_p[j]->Fill(log10((*p_Efrac)[j]));
                deuteron_tree->GetEntry(entry); if ((*d_RMSVec)[0] > 15 && (*d_RMSVec)[1] > 15) h1_d[j]->Fill(log10((*d_Efrac)[j]));
                electron_tree->GetEntry(entry); if ((*e_RMSVec)[0] > 15 && (*e_RMSVec)[1] > 15) h1_e[j]->Fill(log10((*e_Efrac)[j]));
                helium4_tree->GetEntry(entry);  if ((*h_RMSVec)[0] > 15 && (*h_RMSVec)[1] > 15) h1_h[j]->Fill(log10((*h_Efrac)[j]));
                helium3_tree->GetEntry(entry);  if ((*H_RMSVec)[0] > 15 && (*H_RMSVec)[1] > 15) h1_H[j]->Fill(log10((*H_Efrac)[j]));
                carbon_tree->GetEntry(entry);   if ((*c_RMSVec)[0] > 15 && (*c_RMSVec)[1] > 15) h1_c[j]->Fill(log10((*c_Efrac)[j]));
            }
        ...
        legend1->Draw();       
        c1->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/Longti_MonoE/PDF/Longti_Efrac_%dGeV.pdf",int(Energy[i])));
        ...
        c2->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/Longti_MonoE/PDF/Longti_Efrac_BGOLayer_%dGeV.pdf",int(Energy[i])));
    }
