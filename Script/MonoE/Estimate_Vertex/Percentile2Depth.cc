...
void Percentile2Depth()
{
    ...
    // for (int k =18; k < 19; k++)
    {
        ...
        for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
        {
            ...
            if(d_FI_Lay>= 0 && d_FI_Lay <= 3 ) 
            {h1_d_int_Sel     ->Fill((d_FI_Dep/25.5 - sigmoid->GetParameter(2) )/sigmoid->GetParameter(3));}
        ...
        c3->SaveAs(Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Estimate_Vertex/Depth_%dGeV.pdf",Energy_Name[k]));
    }

    auto write_file = new TFile("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Estimate_Vertex/DepthEst_Fitting.root", "RECREATE");
    write_file->cd();
