    ...
    for (int i = 0; i < 1; i++) // Energy
    {
        ...
        for (Long64_t entry = 0; entry < 1; ++entry)
        {
            ...
            for (int k = 0; k < 14; k ++)
            {
                ...
                {
                    int j = k * 22 + i;
                    cout << " Layer = " << k << " ,bar = " << i << " Energy " <<  (*p_EnergyVec)[j] << endl;
                    p_E_total += (*p_EnergyVec)[j];
            //         auto p_start = p_EnergyVec->begin() + k;  auto p_end = (k + 22 < p_EnergyVec->size() ) ? p_start + 22 : p_EnergyVec->end();  p_maxVal[k] = *std::max_element(p_start, p_end); 
            //         auto d_start = d_EnergyVec->begin() + k;  auto d_end = (k + 22 < d_EnergyVec->size() ) ? d_start + 22 : d_EnergyVec->end();  d_maxVal[k] = *std::max_element(d_start, d_end); 
                }                

            //     if((*p_RMSVec)[0]>15 && (*p_RMSVec)[1]>15) 
            //     {
            //         sum_p += p_maxVal[k];
            //         h1_p[k]->Fill(p_maxVal[k]/p_E_total);                  
            //         hC_p[k]->Fill(sum_p/p_E_total);
            //         if (sum_p / p_E_total > 1.0)
            //         {
            //             std::cout << "entry = " << entry 
            //                     << ", sum_p = " << sum_p 
            //                     << ", p_E_total = " << p_E_total 
            //                     << ", ratio = " << sum_p / p_E_total << std::endl;
            //         }
            //     }
            ...
            //     {
            //         sum_d += d_maxVal[k];
            //         h1_d[int(k/22)]->Fill((d_maxVal[k]/d_E_total));
            //         hC_d[int(k/22)]->Fill(sum_d/d_E_total);
            //     }
            //     cout << " bar " << k << " Layer " << k <<endl; 
            //     cout << " max = "<< p_maxVal[k] <<  endl;
            }
            cout <<  p_E_total <<  endl;
        }
