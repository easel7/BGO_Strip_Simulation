void Lateral_RMS()
{
    ...
    for (int i = 0; i < 19; i++) // Energy
    {
        ...
        for (int j = 0; j < 14; j++) // Sub Fig
        {
            ...
            h1_d[j] = new TH1D(Form("h1_d[%d]",j), Form("h1_d[%d]",j),75,0,150);  
            h1_e[j] = new TH1D(Form("h1_e[%d]",j), Form("h1_e[%d]",j),75,0,150);  
            h1_h[j] = new TH1D(Form("h1_h[%d]",j), Form("h1_h[%d]",j),75,0,150);  
            h1_H[j] = new TH1D(Form("h1_H[%d]",j), Form("h1_H[%d]",j),75,0,150);  
            h1_c[j] = new TH1D(Form("h1_c[%d]",j), Form("h1_c[%d]",j),75,0,150);  

            for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
            {
                proton_tree->GetEntry(entry);    if((*p_RMSVec)[0]>15 && (*p_RMSVec)[1]>15) /*if ((*p_EnergyVec)[0] > 0.23 && (*p_EnergyVec)[1] > 0.23 && (*p_EnergyVec)[2] > 0.23 && (*p_EnergyVec)[0] > 0.046)*/  h1_p[j]->Fill((*p_RMSVec)[j]);
                deuteron_tree->GetEntry(entry);  if((*d_RMSVec)[0]>15 && (*d_RMSVec)[1]>15) /*if ((*d_EnergyVec)[0] > 0.23 && (*d_EnergyVec)[1] > 0.23 && (*d_EnergyVec)[2] > 0.23 && (*d_EnergyVec)[0] > 0.046)*/  h1_d[j]->Fill((*d_RMSVec)[j]);
                electron_tree->GetEntry(entry);  if((*e_RMSVec)[0]>15 && (*e_RMSVec)[1]>15) /*if ((*e_EnergyVec)[0] > 0.23 && (*e_EnergyVec)[1] > 0.23 && (*e_EnergyVec)[2] > 0.23 && (*e_EnergyVec)[0] > 0.046)*/  h1_e[j]->Fill((*e_RMSVec)[j]);
                helium4_tree->GetEntry(entry);   if((*h_RMSVec)[0]>15 && (*h_RMSVec)[1]>15) /*if ((*h_EnergyVec)[0] > 0.23 && (*h_EnergyVec)[1] > 0.23 && (*h_EnergyVec)[2] > 0.23 && (*h_EnergyVec)[0] > 0.046)*/  h1_h[j]->Fill((*h_RMSVec)[j]);
                helium3_tree->GetEntry(entry);   if((*H_RMSVec)[0]>15 && (*H_RMSVec)[1]>15) /*if ((*H_EnergyVec)[0] > 0.23 && (*H_EnergyVec)[1] > 0.23 && (*H_EnergyVec)[2] > 0.23 && (*H_EnergyVec)[0] > 0.046)*/  h1_H[j]->Fill((*H_RMSVec)[j]);
                carbon_tree->GetEntry(entry);    if((*c_RMSVec)[0]>15 && (*c_RMSVec)[1]>15) /*if ((*c_EnergyVec)[0] > 0.23 && (*c_EnergyVec)[1] > 0.23 && (*c_EnergyVec)[2] > 0.23 && (*c_EnergyVec)[0] > 0.046)*/  h1_c[j]->Fill((*c_RMSVec)[j]);
            }
        ...
        legend1->Draw();       
        c1->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/MonoE/Lateral_MonoE/Lateral_RMS_%dGeV.pdf",int(Energy[i])));
