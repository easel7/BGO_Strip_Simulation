void Rm_Single()
{
    ...
    auto h1_p = new TH1D("h1_p","h1_p",75,0,150);      double p_maxVal;
    auto h1_d = new TH1D("h1_d","h1_d",75,0,150);      double d_maxVal;
    auto h1_e = new TH1D("h1_e","h1_e",75,0,150);      double e_maxVal;
    auto h1_h = new TH1D("h1_h","h1_h",75,0,150);      double h_maxVal;
    auto h1_H = new TH1D("h1_H","h1_H",75,0,150);      double H_maxVal;
    auto h1_c = new TH1D("h1_c","h1_c",75,0,150);      double c_maxVal;
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        for (size_t i = 22*j; i < 22*(j+1); i += 22)
        {  
            auto p_start = p_EnergyVec->begin() + i;  auto p_end = (i + 22 < p_EnergyVec->size() ) ? p_start + 22 : p_EnergyVec->end();  p_maxVal = *std::max_element(p_start, p_end); if(p_Total_E > pow(10,1+0.2*k) && p_Total_E <= pow(10,1.2+0.2*k)s /* && p_First_Had_Layer==0 */) { h1_p->Fill(log10(p_maxVal/p_Total_E)); }
            auto d_start = d_EnergyVec->begin() + i;  auto d_end = (i + 22 < d_EnergyVec->size() ) ? d_start + 22 : d_EnergyVec->end();  d_maxVal = *std::max_element(d_start, d_end); if(d_Total_E > pow(10,1+0.2*k) && d_Total_E <= pow(10,1.2+0.2*k)s /* && d_First_Had_Layer==0)*/) { h1_d->Fill(log10(d_maxVal/d_Total_E)); }
            auto e_start = e_EnergyVec->begin() + i;  auto e_end = (i + 22 < e_EnergyVec->size() ) ? e_start + 22 : e_EnergyVec->end();  e_maxVal = *std::max_element(e_start, e_end); if(e_Total_E > pow(10,1+0.2*k) && e_Total_E <= pow(10,1.2+0.2*k)s /* && e_First_Had_Layer==0)*/) { h1_e->Fill(log10(e_maxVal/e_Total_E)); }
            auto h_start = h_EnergyVec->begin() + i;  auto h_end = (i + 22 < h_EnergyVec->size() ) ? h_start + 22 : h_EnergyVec->end();  h_maxVal = *std::max_element(h_start, h_end); if(h_Total_E > pow(10,1+0.2*k) && h_Total_E <= pow(10,1.2+0.2*k)s /* && h_First_Had_Layer==0)*/) { h1_h->Fill(log10(h_maxVal/h_Total_E)); }
            auto H_start = H_EnergyVec->begin() + i;  auto H_end = (i + 22 < H_EnergyVec->size() ) ? H_start + 22 : H_EnergyVec->end();  H_maxVal = *std::max_element(H_start, H_end); if(H_Total_E > pow(10,1+0.2*k) && H_Total_E <= pow(10,1.2+0.2*k)s /* && H_First_Had_Layer==0)*/) { h1_H->Fill(log10(H_maxVal/H_Total_E)); }
            auto c_start = c_EnergyVec->begin() + i;  auto c_end = (i + 22 < c_EnergyVec->size() ) ? c_start + 22 : c_EnergyVec->end();  c_maxVal = *std::max_element(c_start, c_end); if(c_Total_E > pow(10,1+0.2*k) && c_Total_E <= pow(10,1.2+0.2*k)s /* && c_First_Had_Layer==0)*/) { h1_c->Fill(log10(c_maxVal/c_Total_E)); }
        }
    ...
    h1_p->GetYaxis()->SetRangeUser(0,0.25);h1_p->SetTitleSetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]; log_{10}(Rm) = log_{10}(Max Energy Deposit bar in L%d/ Total Deposit);Count", 1+0.2*k ,1.2+0.2*k ,j ));
