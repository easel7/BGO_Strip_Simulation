void Longti_Efrac_Single()
{
    ...
    {
        proton_tree->GetEntry(entry);   if (p_Total_E > pow(10,1+0.2*k) && p_Total_E <= pow(10,1.2+0.2*k) ) h1_p->Fill(log10((*p_Efrac)[j]));
        deuteron_tree->GetEntry(entry); if (d_Total_E > pow(10,1+0.2*k) && d_Total_E <= pow(10,1.2+0.2*k) ) h1_d->Fill(log10((*d_Efrac)[j]));
        electron_tree->GetEntry(entry); if (e_Total_E > pow(10,1+0.2*k) && e_Total_E <= pow(10,1.2+0.2*k) ) h1_e->Fill(log10((*e_Efrac)[j]));
        helium4_tree->GetEntry(entry);  if (h_Total_E > pow(10,1+0.2*k) && h_Total_E <= pow(10,1.2+0.2*k) ) h1_h->Fill(log10((*h_Efrac)[j]));
        helium3_tree->GetEntry(entry);  if (H_Total_E > pow(10,1+0.2*k) && H_Total_E <= pow(10,1.2+0.2*k) ) h1_H->Fill(log10((*H_Efrac)[j]));
        carbon_tree->GetEntry(entry);   if (c_Total_E > pow(10,1+0.2*k) && c_Total_E <= pow(10,1.2+0.2*k) ) h1_c->Fill(log10((*c_Efrac)[j]));
    }
    ...
    h1_p->GetYaxis()->SetRangeUser(0,0.25);h1_p->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] EdepRatio Distrubution in L%d;log_{10}(Energy Deposit in Layer/ Total Deposit);Normalized Count", 1+0.2*k ,1.2+0.2*k ,j ));
