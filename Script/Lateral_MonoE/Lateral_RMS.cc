void Lateral_RMS()
{
    ...
    for (int i = 0; i < 19; i++) // Energy
    {
        ...
        for (int j = 0; j < 14; j++) // Sub Fig
        {
            ...
            {
                proton_tree->GetEntry(entry);  /*if ((*p_EnergyVec)[0] > 0.23 && (*p_EnergyVec)[1] > 0.23 && (*p_EnergyVec)[2] > 0.23 && (*p_EnergyVec)[0] > 0.046)*/  h1_p[j]->Fill((*p_RMSVec)[j]);
                deuteron_tree->GetEntry(entry);/*if ((*d_EnergyVec)[0] > 0.23 && (*d_EnergyVec)[1] > 0.23 && (*d_EnergyVec)[2] > 0.23 && (*d_EnergyVec)[0] > 0.046)*/  h1_d[j]->Fill((*d_RMSVec)[j]);
                electron_tree->GetEntry(entry);/*if ((*e_EnergyVec)[0] > 0.23 && (*e_EnergyVec)[1] > 0.23 && (*e_EnergyVec)[2] > 0.23 && (*e_EnergyVec)[0] > 0.046)*/  h1_e[j]->Fill((*e_RMSVec)[j]);
                helium4_tree->GetEntry(entry); /*if ((*h_EnergyVec)[0] > 0.23 && (*h_EnergyVec)[1] > 0.23 && (*h_EnergyVec)[2] > 0.23 && (*h_EnergyVec)[0] > 0.046)*/  h1_h[j]->Fill((*h_RMSVec)[j]);
                helium3_tree->GetEntry(entry); /*if ((*H_EnergyVec)[0] > 0.23 && (*H_EnergyVec)[1] > 0.23 && (*H_EnergyVec)[2] > 0.23 && (*H_EnergyVec)[0] > 0.046)*/  h1_H[j]->Fill((*H_RMSVec)[j]);
                carbon_tree->GetEntry(entry);  /*if ((*c_EnergyVec)[0] > 0.23 && (*c_EnergyVec)[1] > 0.23 && (*c_EnergyVec)[2] > 0.23 && (*c_EnergyVec)[0] > 0.046)*/  h1_c[j]->Fill((*c_RMSVec)[j]);
            }
