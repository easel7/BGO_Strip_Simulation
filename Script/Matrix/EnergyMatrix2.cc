void EnergyMatrix2()
{
    ...
    auto *h2_p = new TH2D("h2_p","h2_p",30,1,4.0,40,0,4.0); auto *h1_p = new TH2D("h1_p","h1_p",30,1,4.0,40,0,4.0);
    auto *h2_d = new TH2D("h2_d","h2_d",30,1,4.0,40,0,4.0); auto *h1_d = new TH2D("h1_d","h1_d",30,1,4.0,40,0,4.0);

    auto *hR_p = new TH2D("hR_p","hR_p",30,1,4.0,40,-2,2);  auto *hr_p = new TH2D("hr_p","hr_p",30,1,4.0,40,-2,2); 
    auto *hR_d = new TH2D("hR_d","hR_d",30,1,4.0,40,-2,2);  auto *hr_d = new TH2D("hr_d","hr_d",30,1,4.0,40,-2,2);
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h2_p->Fill( log10(p_Energy), log10(p_Total_E), p_weight); 
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h2_d->Fill(log10(d_Energy), log10(d_Total_E),  d_weight); 
