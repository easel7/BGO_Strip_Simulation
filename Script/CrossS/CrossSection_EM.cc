void CrossSection_EM()
{
    ...
    for (int i = 0; i < 19; i++)
    {
        ...
        h1_p[i] = new TH1D(Form("h1_p[%d]",i),Form("h1_p[%d]",i),50,0,100);
        proton_tree->Draw(Form("First_Depth>>h1_p[%d]",i),EM,"");
        fitFunc_p[i] = new TF1(Form("fitFunc_p[%d]",i), "[0]*exp(-x/[1])", 0, 60);
        ...
        h1_d[i] = new TH1D(Form("h1_d[%d]",i),Form("h1_d[%d]",i),40,0,100);
        deuteron_tree->Draw(Form("First_Depth>>h1_d[%d]",i),EM, "");
        fitFunc_d[i] = new TF1(Form("fitFunc_d[%d]",i), "[0]*exp(-x/[1])", 0, 60);
        ...
        h1_e[i] = new TH1D(Form("h1_e[%d]",i),Form("h1_e[%d]",i),40,0,20);
        fitFunc_e[i] = new TF1(Form("fitFunc_e[%d]",i), "[0]*exp(-x/[1])", 0, 60);
        fitFunc_e[i]->SetParameters(100, 15); 
        electron_tree->Draw(Form("First_Depth>>h1_e[%d]",i),EM, "");
        ...
        h1_h[i] = new TH1D(Form("h1_h[%d]",i),Form("h1_h[%d]",i),20,0,40);
        fitFunc_h[i] = new TF1(Form("fitFunc_h[%d]",i), "[0]*exp(-x/[1])", 0, 60);
        fitFunc_h[i]->SetParameters(100, 15); 
        helium4_tree->Draw(Form("First_Depth>>h1_h[%d]",i),EM, "");
        ...
        fitFunc_H[i] = new TF1(Form("fitFunc_H[%d]",i), "[0]*exp(-x/[1])", 0, 60);
        fitFunc_H[i]->SetParameters(100, 15); 
        helium3_tree->Draw(Form("First_Depth>>h1_H[%d]",i),EM, "");
        ...
        h1_c[i] = new TH1D(Form("h1_c[%d]",i),Form("h1_c[%d]",i),20,0,10);
        fitFunc_c[i] = new TF1(Form("fitFunc_c[%d]",i), "[0]*exp(-x/[1])", 0, 10);
        fitFunc_c[i]->SetParameters(1000, 1); 
        carbon_tree->Draw(Form("First_Depth>>h1_c[%d]",i),EM, "");
        ...
        double carbon_nsigma     = fitFunc_c[i]->GetParameter(1);
        double carbon_nsigma_err = fitFunc_c[i]->GetParError(1);

        Carbon_Section[i] = 1 / (carbon_nsigma * n_BGO) * 1e25;
        Carbon_Section_Err[i] = Carbon_Section[i] * carbon_nsigma_err / carbon_nsigma;
        cout << "Carbon " << Energy[i] << " GeV: Sigma = " << Carbon_Section[i] << " ± " << Carbon_Section_Err[i] << " barn" << endl;


        c1->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/CrossS/CrossSection_EM_%dGeV.pdf",int(Energy[i])) );
    }
    
    auto c0 = new TCanvas("c0","c0",1200,1200);
    auto gre_p = new TGraphErrors(19, Energy, Proton_Section, Energy_Err, Proton_Section_Err);
    gre_p->SetTitle("Fitted Cross Section vs Energy; Kinetic Energy (GeV); Cross Section per Atom (barn)");
    ...
    gre_p->Draw("AP");
    gre_p->GetYaxis()->SetRangeUser(-10, 350);
    ...
    gre_d->Draw("AP");
    gre_d->GetYaxis()->SetRangeUser(9.5, 11);
    ...
    c0->SaveAs("/Users/xiongzheng/software/B4/B4e/Script/CrossS/CrossSection_EM.pdf");
