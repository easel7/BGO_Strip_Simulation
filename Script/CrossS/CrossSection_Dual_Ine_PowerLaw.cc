void CrossSection_Dual_Ine_PowerLaw()
{
    ...
    auto hist_int = new TH2D("hist_int","hist_int",7,Energy_Bin,28,Ratio_Edge);
    auto hist_sur = new TH2D("hist_sur","hist_sur",7,Energy_Bin,28,Ratio_Edge);
    ...
    // for(int j=4 ;j< 5 ; j++) // Energy
    {
        ...
        for (int k =0; k < 27; k++)
        {
            ...
            else if (k < 19)          Ratio[k] = (k - 9 + 1) * 0.01;
            else                      Ratio[k] = (k - 18 + 1) * 0.1;  
            cout << Ratio[k] << " , " << 1-Ratio[k] <<  endl;
            ...
            tree_p->Draw("First_Ine_Depth>>h_p_tot",Form("(energy_res>%.f && energy_res<%.f)",Energy_Dep[j],Energy_Dep[j+1]),"weight*1e4");
            tree_p->Draw("First_Ine_Depth>>h_p_int",Form("(energy_res>%.f && energy_res<%.f)",Energy_Dep[j],Energy_Dep[j+1]),"weight*1e4");
            tree_p->Draw("First_Ine_Depth>>C_p_int",Form("(energy_res>%.f && energy_res<%.f)",Energy_Dep[j],Energy_Dep[j+1]),"weight*1e4");
            ...
            tree_d->Draw("First_Ine_Depth>>h_d_tot",Form("(energy_res>%.f && energy_res<%.f)",Energy_Dep[j],Energy_Dep[j+1]),"weight*1e4");
            tree_d->Draw("First_Ine_Depth>>h_d_int",Form("(energy_res>%.f && energy_res<%.f)",Energy_Dep[j],Energy_Dep[j+1]),"weight*1e4");
            ...
            h_p_int->Scale( 1-Ratio[k]);    h_p_tot->Scale(1-Ratio[k]);
            h_d_int->Scale( Ratio[k]  );    h_d_tot->Scale(Ratio[k]);
            ...
            chi2_int->SetPoint(k,Ratio[k],Chi2_combine_int - (Chi2_mixture_int + Chi2_proton_int));
            chi2_sur->SetPoint(k,Ratio[k],Chi2_combine_sur - (Chi2_mixture_sur + Chi2_proton_sur));
            gre_sur->SetPoint(k,Ratio[k],mixture_length);
            gre_sur->SetPointError(k,0,mixture_length_err);
            gre_int->SetPoint(k,Ratio[k],mixture_length1);
            gre_int->SetPointError(k,0,mixture_length1_err);
            gre_p_sur->SetPoint(k,Ratio[k],lambda1);
            gre_p_sur->SetPointError(k,0,lambda1_err);
            gre_p_int->SetPoint(k,Ratio[k],lambda3);
            gre_p_int->SetPointError(k,0,lambda3_err);
            gre_d_sur->SetPoint(k,Ratio[k],lambda2);
            gre_d_sur->SetPointError(k,0,lambda2_err);
            gre_d_int->SetPoint(k,Ratio[k],lambda4);
            gre_d_int->SetPointError(k,0,lambda4_err);
        }
