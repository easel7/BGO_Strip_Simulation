void Longti_COG_from_Interaction()
{
    int p_First_Had_Layer; int p_First_Had_Type; double p_Total_E;  std::vector<double>* p_RMSVec = nullptr;    std::vector<double>* p_L_EnergyVec = nullptr;    std::vector<double>* p_Efrac = nullptr; double p_weight;
    int d_First_Had_Layer; int d_First_Had_Type; double d_Total_E;  std::vector<double>* d_RMSVec = nullptr;    std::vector<double>* d_L_EnergyVec = nullptr;    std::vector<double>* d_Efrac = nullptr; double d_weight;
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        {
            sum_p += (*p_L_EnergyVec)[i] * (i-p_First_Had_Layer) / p_Total_E;
            h1_p[p_energy_index][i-p_First_Had_Layer]->Fill(sum_p);
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        {
            sum_d += (*d_L_EnergyVec)[i] * (i- d_First_Had_Layer) /d_Total_E;
            h1_d[d_energy_index][i- d_First_Had_Layer]->Fill(sum_d);
