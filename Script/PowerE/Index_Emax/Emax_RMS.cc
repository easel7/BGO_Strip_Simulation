void Emax_RMS()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h1_p[p_energy_index][p_First_Had_Layer]->Fill( (*p_RMSVec)[p_maxIndex] );
        h1_p_inter[p_energy_index]->Fill( (*p_RMSVec)[p_maxIndex]) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_First_Had_Layer]->Fill( (*d_RMSVec)[d_maxIndex] );
        h1_d_inter[d_energy_index]->Fill( (*d_RMSVec)[d_maxIndex] ) ;
    }
