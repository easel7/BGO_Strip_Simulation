...
void Peak_JM()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        auto p_Bar_start = p_EnergyVec->begin() + seg_peak_idx *22 ;  auto p_Bar_end = p_EnergyVec->begin() + (seg_peak_idx +1)*22;
        double p_Bar_maxVal = *std::max_element(p_Bar_start, p_Bar_end);

        h1_p[p_energy_index][p_First_Had_Layer]->Fill( (p_Bar_maxVal /(*p_L_EnergyVec)[seg_peak_idx]) );
        h1_p_inter[p_energy_index]->Fill( (p_Bar_maxVal / (*p_L_EnergyVec)[seg_peak_idx]) );
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        auto d_Bar_start = d_EnergyVec->begin() + seg_peak_idx *22 ;  auto d_Bar_end = d_EnergyVec->begin() + (seg_peak_idx +1)*22;
        double d_Bar_maxVal = *std::max_element(d_Bar_start, d_Bar_end); 

        h1_d[d_energy_index][d_First_Had_Layer]->Fill( (d_Bar_maxVal / (*d_L_EnergyVec)[seg_peak_idx]) );
        h1_d_inter[d_energy_index]->Fill( (d_Bar_maxVal /(*d_L_EnergyVec)[seg_peak_idx]) ) ;
    }
