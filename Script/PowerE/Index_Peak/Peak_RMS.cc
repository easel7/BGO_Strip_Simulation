...
void Peak_RMS()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h1_p[p_energy_index][p_First_Had_Layer]->Fill( (*p_RMSVec)[seg_start_idx] );
        h1_p_inter[p_energy_index]->Fill( (*p_RMSVec)[seg_start_idx]) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_First_Had_Layer]->Fill( (*d_RMSVec)[seg_start_idx] );
        h1_d_inter[d_energy_index]->Fill( (*d_RMSVec)[seg_start_idx] ) ;
    }
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;RMS (mm);Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
