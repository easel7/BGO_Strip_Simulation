...
void Percentile2Efrac()
{
    ...
    auto h1_p_int = new TH1D("h1_p_int","h1_p_int",50,-4,1);  
    auto h1_d_int = new TH1D("h1_d_int","h1_d_int",50,-4,1);  
    ...
    for(int i =0 ; i<15 ; i++)  // Deposit Energy Bin
    {
        ...
        h1_p_inter[i] =new TH1D(Form("h1_p_inter[%d]",i),Form("h1_p_inter[%d]",i),50,-4,1);  
        h1_d_inter[i] =new TH1D(Form("h1_d_inter[%d]",i),Form("h1_d_inter[%d]",i),50,-4,1);  
        for( int j= 0; j<14 ;j++)
        {
            h1_p[i][j] = new TH1D(Form("h1_p[%d][%d]",i,j), Form("h1_p[%d][%d]",i,j),50,-4,1);  
            h1_d[i][j] = new TH1D(Form("h1_d[%d][%d]",i,j), Form("h1_d[%d][%d]",i,j),50,-4,1);  
            Layer[j] = 0.5 + j;
            Layer_Err[j] = 0.5;
        }
        if (i<14)
        {
            h1_p_Lay[i] =new TH1D(Form("h1_p_Lay[%d]",i),Form("h1_p_Lay[%d]",i),50,-4,1);  
            h1_d_Lay[i] =new TH1D(Form("h1_d_Lay[%d]",i),Form("h1_d_Lay[%d]",i),50,-4,1);  
        }
    }
    ...
    {        
        ...
        // h1_p[p_energy_index][p_FI_Lay]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        // h1_p_inter[p_energy_index]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        h1_p_int->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        h1_p_Lay[p_FI_Lay]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        // h1_d[d_energy_index][d_FI_Lay]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        // h1_d_inter[d_energy_index]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        h1_d_int->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
        h1_d_Lay[d_FI_Lay]->Fill(log10( sigmoid->Eval(sigmoid->GetParameter(2)/sigmoid->Eval(14)) ) );
    }
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;log10(F(Xmid)/F(14));Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
        ...
        h1_p_inter[i]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] Stack Multi Layer;log10(F(Xmid)/F(14));Normalized Count",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
    ...
    h1_p_int->SetTitle(" Stack Multi Layer;log10(F(Xmid)/F(14));Normalized Count");
    h1_p_int->Draw("hist");
    ...
    for (int j = 0; j < 14; j++) // layer
    {
        ...
        h1_p_Lay[j]->SetTitle(" Stack Multi Layer;log10(F(Xmid)/F(14));Normalized Count");
