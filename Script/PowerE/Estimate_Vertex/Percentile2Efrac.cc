...
void Percentile2Efrac()
{
    ...
    {        
        ...
        int Percent2Layer = Inverse_Mod_sigmoid(pow(10,-2.87), Xmid,Slope);

        h1_p[p_energy_index][p_FI_Lay]->Fill(log10((*p_Efrac)[Percent2Layer]));
        h1_p_inter[p_energy_index]->Fill(log10((*p_Efrac)[Percent2Layer])) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_FI_Lay]->Fill(log10((*d_Efrac)[Percent2Layer]));
        h1_d_inter[d_energy_index]->Fill(log10((*d_Efrac)[Percent2Layer])) ;
    }
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;log10(Efrac);Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
        ...
        h1_p_inter[i]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] Stack Multi Layer;log10(Efrac);Normalized Count",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
