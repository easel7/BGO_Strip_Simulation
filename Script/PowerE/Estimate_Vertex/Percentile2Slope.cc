...
void Percentile2Slope()
{
    ...
    {        
        ...
        h1_p[p_energy_index][p_FI_Lay]->Fill(Slope);
        h1_p_inter[p_energy_index]->Fill(Slope);
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        int fit_status = minuit.Migrad();
        if (fit_status != 0) {
            std::cerr << "WARNING: Fit did not converge! Status: " << fit_status << " particle = " << d_particle << " entry : " << entry << std::endl;
        }
        double Ymin, Ymin_err, Ymax, Ymax_err;
        double Slope, Slope_err, Xmid, Xmid_err;
        minuit.GetParameter(0, Ymin, Ymin_err);
        ...
        h1_d[d_energy_index][d_FI_Lay]->Fill(Slope);
        h1_d_inter[d_energy_index]->Fill(Slope);
    }
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;Slope (BGO Layer);Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
        ...
        h1_p_inter[i]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] Stack Multi Layer;Slope (BGO Layer);Normalized Count",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
