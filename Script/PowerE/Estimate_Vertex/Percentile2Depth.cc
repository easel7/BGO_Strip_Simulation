...
void Percentile2Depth()
{
    ...
    for(int i =0 ; i<15 ; i++)  // Deposit Energy Bin
    {
        ...
        h1_p_inter[i] =new TH1D(Form("h1_p_inter[%d]",i),Form("h1_p_inter[%d]",i),70,0,350);  
        h1_d_inter[i] =new TH1D(Form("h1_d_inter[%d]",i),Form("h1_d_inter[%d]",i),70,0,350);  
        for( int j= 0; j<14 ;j++)
        {
            h1_p[i][j] = new TH1D(Form("h1_p[%d][%d]",i,j), Form("h1_p[%d][%d]",i,j),70,0,350);  
    ...
    {        
        ...
        int Percent2Layer = Inverse_Mod_sigmoid(pow(10,-1.67), Xmid,Slope);

        h1_p[p_energy_index][p_FI_Lay]->Fill(p_FI_Dep);
        h1_p_inter[p_energy_index]->Fill(p_FI_Dep) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_FI_Lay]->Fill(d_FI_Dep);
        h1_d_inter[d_energy_index]->Fill(d_FI_Dep) ;
    }
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;Depth(mm);Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
        ...
        h1_p_inter[i]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] Stack Multi Layer;Depth(mm);Normalized Count",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
