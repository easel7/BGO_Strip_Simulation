...
void Percentile2DepthEst_Res_Theory()
{
    ...
    for(int j=0 ;j< 7 ; j++) // Energy
    {
        ...
        // for (int i =26; i < 27; i++)
        {
            ...
            *h1_d_sur[j] = *h1_d_sur_orig;
            ...
            h_2_sur->Draw("E");
            h1_p_sur[j]->Draw("Esame");
            h1_d_sur[j]->Draw("Esame");
            ...
            h1_p_sur[j]->Fit(fitFunc1, "QRS"); 
            h1_d_sur[j]->Fit(fitFunc2, "QSR"); 
            h_2_sur->Fit(fitFunc3, "QRS"); 
            fitFunc1->Draw("same");
            fitFunc2->Draw("same");
            fitFunc3->Draw("same");
            ...
            f1_sur->SetParameters((h2_p_tot[j]->Integral()+h2_d_tot[j]->Integral()), 200);
            f1_sur->FixParameter(1,gre6->Eval(Energy_Dep[j]));
            ...
            if( (Chi2_proton_sur - Chi2_mixture_sur ) <=3.84 && Sur_95_Tag < 0) 
            { 
                CL95_Sur[j] = Ratio[i];
                Sur_95_Tag = 1;
            }
            ...
            f1_int->SetParameters((h2_p_tot[j]->Integral()+h2_d_tot[j]->Integral()), 200);
            f1_int->SetLineColor(kOrange-3);
            f1_int->FixParameter(1,gre6->Eval(Energy_Dep[j]));
            h_2_int->Fit(f1_int,"QSR");
            double Chi2_proton_int    = f1_int->GetChisquare();
            double Chi2_mixture_int   = fitFunc4->GetChisquare();

            if( (Chi2_proton_int - Chi2_mixture_int) <=2.71 && Int_90_Tag < 0) 
            { 
                CL90_Int[j] = Ratio[i];
                CL95_Int[j] = Ratio[i];
                Int_90_Tag = 1;
            }
            if( (Chi2_proton_int - Chi2_mixture_int) <=3.84 && Int_95_Tag < 0) 
            { 
                CL95_Int[j] = Ratio[i];
                Int_95_Tag = 1;
            }
            if (i==49 && Int_95_Tag < 0 )
            {
                CL95_Int[j] = Ratio[i];
                CL90_Int[j] = Ratio[i];
                Int_95_Tag = 1;
            ...
            gre_p_sur->SetPoint(i,Ratio[i],gre6->Eval(Energy_Dep[j]));
            gre_p_sur->SetPointError(i,0,0);
            gre_p_int->SetPoint(i,Ratio[i],gre6->Eval(Energy_Dep[j]));
            gre_p_int->SetPointError(i,0,0);

            gre_d_sur->SetPoint(i,Ratio[i],gre7->Eval(Energy_Dep[j]));
            gre_d_sur->SetPointError(i,0,0);
            gre_d_int->SetPoint(i,Ratio[i],gre7->Eval(Energy_Dep[j]));
        ...
        auto write_file = new TFile(Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Estimate_Vertex/DepthEst_Res_Theory_%d.root",j), "RECREATE");
        write_file->cd();
        h2_p_tot[j]->Write();
        h2_d_tot[j]->Write();
        h1_p_int[j]->Write();
    ...
    auto write_file2 = new TFile("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Estimate_Vertex/DepthEst_Energy_Theory.root", "RECREATE");
