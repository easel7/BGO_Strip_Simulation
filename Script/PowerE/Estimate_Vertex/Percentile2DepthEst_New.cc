...
void Percentile2DepthEst_New()
{
    ...
    auto mean_file    = TFile::Open("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Estimate_Vertex/EnergyEst_New.root");
    ...
    int p_FH_Lay; int p_FH_Type; double p_energy_new;  double p_Energy;  double p_Total_E;    int p_Nhits;std::vector<double>* p_RMSVec = nullptr;    std::vector<double>* p_L_EnergyVec = nullptr;   std::vector<double>* p_EnergyVec = nullptr;   std::vector<double>* p_Efrac = nullptr; double p_weight;
    int d_FH_Lay; int d_FH_Type; double d_energy_new;  double d_Energy;  double d_Total_E;    int d_Nhits;std::vector<double>* d_RMSVec = nullptr;    std::vector<double>* d_L_EnergyVec = nullptr;   std::vector<double>* d_EnergyVec = nullptr;   std::vector<double>* d_Efrac = nullptr; double d_weight;
    ...
    proton_tree->SetBranchAddress("energy_new"     ,&p_energy_new);
    ...
    deuteron_tree->SetBranchAddress("energy_new"     ,&d_energy_new);
    ...
    {        
        ...
        if (p_energy_new < hist_p_2->GetXaxis()->GetXmin() || p_energy_new > hist_p_2->GetXaxis()->GetXmax()) continue;
        int BEST_FIT_MEAN_BIN  = hist_p_2->FindBin(p_energy_new);
    ...
    // for (Long64_t entry = 0; entry < 50000 ; ++entry)
    {
        ...
        if (d_energy_new < hist_p_2->GetXaxis()->GetXmin() || d_energy_new > hist_p_2->GetXaxis()->GetXmax()) continue;
        int BEST_FIT_MEAN_BIN = hist_p_2->FindBin(d_energy_new);
        double BEST_FIT_MEAN     = hist_p_2->GetBinContent(BEST_FIT_MEAN_BIN);
        double sum_d = 0;
        int bar_info[2] = {0};
        double bar_Energy_info[14] = {0};
    ...
    for(int j=0 ;j< 7 ; j++)
    {
        ...
        auto write_file = new TFile(Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Estimate_Vertex/DepthEst_New_%d.root",j), "RECREATE");
        write_file->cd();
        h2_p_tot[j]->Write();
        h2_d_tot[j]->Write();
        h1_p_int[j]->Write();
    ...
    auto write_file2 = new TFile("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Estimate_Vertex/DepthEst_Energy_New.root", "RECREATE");
    write_file2->cd();
    c6->Write();
    c7->Write();
    c8->Write();
