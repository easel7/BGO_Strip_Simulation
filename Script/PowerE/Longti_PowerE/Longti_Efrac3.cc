void Longti_Efrac3()
{
  ...
  TH1D *h1_d[15][14];  TH1D *hC_d[15][14];       TH2D *h2_d[15];      TGraph *gr_deuteron[15];        int point_counter_d = 0;
    ...
    for(int i =0 ; i<15 ; i++)  // Deposit Energy Bin
    {
        ...
        h2_p[i] = new TH2D(Form("h2_p[%d]",i),Form("h2_p[%d]",i),14,0,14,nbins, bin_edges.data());
        h2_d[i] = new TH2D(Form("h2_d[%d]",i),Form("h2_d[%d]",i),14,0,14,nbins, bin_edges.data());
        for( int j= 0; j<14 ;j++)
        {
            ...
            hC_p[i][j] = new TH1D(Form("hC_p[%d][%d]",i,j), Form("hC_p[%d][%d]",i,j),nbins, bin_edges.data());
            hC_d[i][j] = new TH1D(Form("hC_d[%d][%d]",i,j), Form("hC_d[%d][%d]",i,j),nbins, bin_edges.data());
            Layer[j] = 0.5 + j;
            Layer_Err[j] = 0.5;
        }
    }
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        // if ((*p_EnergyVec)[0] > 0.23 && (*p_EnergyVec)[1] > 0.23 && (*p_EnergyVec)[2] > 0.23 && (*p_EnergyVec)[0] > 0.046)
        {
            for (int k = 0; k < p_EnergyVec->size(); k ++)
            {
                ...
                sum_p += (*p_Efrac)[k];
                gr_proton[p_energy_index]->SetPoint(point_counter_p++, k+0.9, sum_p);
                hC_p[p_energy_index][k]->Fill(sum_p);
                h2_p[p_energy_index]->Fill(k,sum_p);
            }
        }
    }
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        // if ((*d_EnergyVec)[0] > 0.23 && (*d_EnergyVec)[1] > 0.23 && (*d_EnergyVec)[2] > 0.23 && (*d_EnergyVec)[0] > 0.046)
        {
            for (int k = 0; k < d_EnergyVec->size(); k ++)
            {
                ...
                sum_d += (*d_Efrac)[k];
                gr_deuteron[d_energy_index]->SetPoint(point_counter_d++, k+1.1, sum_d);
                hC_d[d_energy_index][k]->Fill((sum_d));
                h2_d[d_energy_index]->Fill(k,(sum_d));
            }
            // cout << endl;
        }
    }
    ...
    for (int i = 10; i < 11; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            hC_p[i][j]->SetTitle(Form("Distrubution of #sum_{0}^{L%d} EdepRatio;log_{10}(#sum(Energy Deposit Ratio));Normalized Count",j));
            hC_p[i][j]->Draw("hist");
            ...
            double SUM_p_values[3];  hC_d[i][j]->GetQuantiles(3, SUM_p_values, quantiles);  
            double SUM_d_values[3];  hC_d[i][j]->GetQuantiles(3, SUM_d_values, quantiles);  
            ...
            SUM_Proton_Ratio[j]   = SUM_p_values[1];   SUM_Proton_Ratio_LL[j]   = SUM_p_values[1] - SUM_p_values[0];   SUM_Proton_Ratio_UL[j]   = SUM_p_values[2] - SUM_p_values[1];
            SUM_Deuteron_Ratio[j] = SUM_d_values[1];   SUM_Deuteron_Ratio_LL[j] = SUM_d_values[1] - SUM_d_values[0];   SUM_Deuteron_Ratio_UL[j] = SUM_d_values[2] - SUM_d_values[1];
        }
        c1->cd(15);
        ...
        SUM_gre_d->SetTitle(Form("Deposit Energy [%.2fGeV, %.2fGeV]; BGO Layer; Distrubution of #sum EdepRatio",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
        ...
        gPad->SetLogy(1);
        gr_proton[i]->SetTitle(Form("Deposit Energy [%.2fGeV, %.2fGeV]; BGO Layer; Distrubution of #sum EdepRatio",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
        gr_proton[i]->GetYaxis()->SetRangeUser(1e-5,1);
        gr_proton[i]->GetXaxis()->SetLimits(0,14);
        ...
        h2_p[i]->SetTitle(Form("Deposit Energy [%.2fGeV, %.2fGeV]; BGO Layer; Distrubution of #sum EdepRatio",Energy_LL[i],Energy_UL[i]));
        h2_p[i]->GetYaxis()->SetRangeUser(0,1);
