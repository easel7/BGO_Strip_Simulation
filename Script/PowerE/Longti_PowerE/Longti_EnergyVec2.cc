void Longti_EnergyVec2()
{
    ...
    for(int i =0 ; i<15 ; i++)  // Deposit Energy Bin
    {
        ...
        for( int j= 0; j<14 ;j++)
        {
            h1_p[i][j] = new TH1D(Form("h1_p[%d][%d]",i,j), Form("h1_p[%d][%d]",i,j),60,-2,4);  
            h1_d[i][j] = new TH1D(Form("h1_d[%d][%d]",i,j), Form("h1_d[%d][%d]",i,j),60,-2,4);    
            Layer[j] = 0.5 + j;
            Layer_Err[j] = 0.5;
        }
    }
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        // if ((*p_EnergyVec)[0] > 0.23 && (*p_EnergyVec)[1] > 0.23 && (*p_EnergyVec)[2] > 0.23 && (*p_EnergyVec)[0] > 0.046)
        {
            ...
            {
                // h1_p[p_energy_index][k]->Fill((*p_EnergyVec)[k]);
                h1_p[p_energy_index][k]->Fill(log10((*p_EnergyVec)[k]),p_weight);
            }
        }
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        // if ((*d_EnergyVec)[0] > 0.23 && (*d_EnergyVec)[1] > 0.23 && (*d_EnergyVec)[2] > 0.23 && (*d_EnergyVec)[0] > 0.046)
        {
            for (int k = 0; k < d_EnergyVec->size(); k ++)
            {
                ...
                // h1_d[d_energy_index][k]->Fill((*d_EnergyVec)[k]);
                h1_d[d_energy_index][k]->Fill(log10((*d_EnergyVec)[k]),d_weight);
            }
            // cout << endl;
        }
    }
