void Longti_Efrac()
{
    ...
    for (int i = 0; i < 13; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_e[i][j]->GetYaxis()->SetRangeUser(0,0.25);h1_e[j]->SetTitleSetTitle(Form("10^%.f - %.f GeV Deposit EdepRatio Distrubution in L%d;log_{10}(Energy Deposit in Layer/ Total Deposit);Normalized Count", 1+0.2*k ,1.2+0.2*k ,j ));
            h1_e[i][j]->Draw();
            h1_H[i][j]->Draw("same");
            h1_c[i][j]->Draw("same");
            h1_h[i][j]->Draw("same");
