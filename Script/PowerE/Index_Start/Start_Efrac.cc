...
void Start_Efrac()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h1_p[p_energy_index][p_First_Had_Layer]->Fill( log10((*p_Efrac)[seg_start_idx]) );
        h1_p_inter[p_energy_index]->Fill( log10((*p_Efrac)[seg_start_idx])) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_First_Had_Layer]->Fill( log10((*d_Efrac)[seg_start_idx]) );
        h1_d_inter[d_energy_index]->Fill( log10((*d_Efrac)[seg_start_idx]) ) ;
    }
