void Longti_EmaxEtot_Interaction()
{
    ...
    for(int i =0 ; i<15 ; i++)  // Deposit Energy Bin
    {
        ...
        h1_p_inter[i] =new TH1D(Form("h1_p_inter[%d]",i),Form("h1_p_inter[%d]",i), 48,-1.2,0);  
        h1_d_inter[i] =new TH1D(Form("h1_d_inter[%d]",i),Form("h1_d_inter[%d]",i), 48,-1.2,0);  

        for( int j= 0; j<14 ;j++)
        {
            h1_p[i][j] = new TH1D(Form("h1_p[%d][%d]",i,j), Form("h1_p[%d][%d]",i,j),48,-1.2,0);  
            h1_d[i][j] = new TH1D(Form("h1_d[%d][%d]",i,j), Form("h1_d[%d][%d]",i,j),48,-1.2,0);    
            Layer[j] = 0.5 + j;
            Layer_Err[j] = 0.5;
        }
    }
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h1_p[p_energy_index][p_First_Had_Layer]->Fill(log10(p_maxVal/p_Total_E));
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        h1_d[d_energy_index][d_First_Had_Layer]->Fill(log10(d_maxVal/d_Total_E));
    ...
    for (int i = 9; i < 10; i++) // Deposit Energy Bin
    {
        ...
        for (int j = 0; j < 14; j++) // layer
        {
            ...
            h1_p[i][j]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV]Interaction happened in L%d;log10(Emax/Edep);Normalized Count", pow(10,Energy_LL[i]),pow(10,Energy_UL[i]),j ));
            h1_p[i][j]->Draw("hist");
        ...
        h1_p_inter[i]->SetTitle(Form("Deposit Energy[%.2fGeV, %.2fGeV] Stack Multi Layer;log10(Emax/Edep);Normalized Count",pow(10,Energy_LL[i]),pow(10,Energy_UL[i])));
