...
void Longti_Start_Peak_Len_Interaction()
{
    ...
    for (Long64_t entry = 0; entry < proton_tree->GetEntries(); ++entry)
    {
        ...
        h1_p[p_energy_index][p_First_Had_Layer]->Fill(seg_peak_idx-seg_start_idx);
        h1_p_inter[p_energy_index]->Fill(seg_peak_idx-seg_start_idx) ;
    }
    ...
    for (Long64_t entry = 0; entry < deuteron_tree->GetEntries(); ++entry)
    {
        ...
        FindMaxPositiveSegment(bar_Change_info,14,seg_sum,seg_len,seg_start_idx);
        FindMaxValueInPositiveSegment(bar_Change_info,seg_start_idx,seg_len,seg_peak_value,seg_peak_idx);
        h1_d[d_energy_index][d_First_Had_Layer]->Fill(seg_peak_idx-seg_start_idx);
        h1_d_inter[d_energy_index]->Fill(seg_peak_idx-seg_start_idx) ;
    }
