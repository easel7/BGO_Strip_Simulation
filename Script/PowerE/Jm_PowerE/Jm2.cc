void Jm2()
{
    ...
    // for (Long64_t entry = 0; entry < 1; ++entry)
    {
        ...
        if( (*p_L_EnergyVec)[0]>0.23 && (*p_L_EnergyVec)[1]>0.23 && (*p_RMSVec)[2]<40 && (*p_RMSVec)[3]<40 )  // 
        {
            for (size_t k = 0; k < p_EnergyVec->size(); k += 22)
            {
                ...
                gr_proton[p_energy_index]->SetPoint(point_counter_p++, index+0.9, sum_p / sum_p_total);
                h2_p[p_energy_index]->Fill(index,log10(sum_p / sum_p_total));
            }
        }
    }
    ...
    // for (Long64_t entry = 0; entry < 1; ++entry)
    {
        ...
        if((*d_L_EnergyVec)[0]>0.23 && (*d_L_EnergyVec)[1]>0.23 && (*d_RMSVec)[2]<40 && (*d_RMSVec)[3]<40) // 
        {
            for (size_t k = 0; k < d_EnergyVec->size(); k += 22)
            {
                ...
                gr_deuteron[d_energy_index]->SetPoint(point_counter_d++, index+1.1, sum_d / sum_d_total);
                h2_d[d_energy_index]->Fill(index,log10(sum_d / sum_d_total));
            }
            // cout << endl;
        }
        // cout << "Next " << endl;
    ...
    for(int i = 10 ; i<11 ; i++) // Energy
    {
        ...
        legend3->Draw();
        c3->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Jm_PowerE/PDF/Jm_BGOLayer_DP_%.2f_%.2f.pdf",Energy_LL[i],Energy_UL[i]));
        ...
        legend4->Draw();
        c4->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Jm_PowerE/CDF/Jm_BGOLayer_SUM_DP_%.2f_%.2f.pdf",Energy_LL[i],Energy_UL[i]));
        ...
        legend5->Draw();
        c5->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Jm_PowerE/CDF/Jm_BGOLayer_SCAT_DP_%.2f_%.2f.pdf",Energy_LL[i],Energy_UL[i]));
        ...
        axis_top->Draw();
        legend5->Draw();
        c6->SaveAs( Form("/Users/xiongzheng/software/B4/B4e/Script/PowerE/Jm_PowerE/CDF/Jm_BGOLayer_CANDLE_DP_%.2f_%.2f.pdf",Energy_LL[i],Energy_UL[i]));
    }
}
